(function() {
  ...
  render_route_layer = function(itinerary, routeLayer) {
    ...
    for (_i = 0, _len = legs.length; _i < _len; _i++) {
      ...
      _results.push((function(leg) {
        var color, dashArray, icon, label, last_stop, marker, point, points, polyline, route, secondsCounter, stop, uid, _ref1, _ref2;
        ...
        if (leg.routeType != null) {
          ...
          $.getJSON(citynavi.config.siri_url, {
            ...
            for (_j = 0, _len1 = _ref3.length; _j < _len1; _j++) {
              vehicle = _ref3[_j];
              handle_vehicle_update(true, siri_to_live(vehicle), route);
            }
            ...
            return (_ref4 = citynavi.realtime) != null ? _ref4.subscribe_route(leg.routeId, function(msg) {
              return handle_vehicle_update(false, msg, route);
            }) : void 0;
          });
        }
        return polyline;
  ...
  handle_vehicle_update = function(initial, msg, route) {
    var icon, id, interpolation, mode, old_pos, pos, mpos, route, routeType, steps, _ref1;
    var lines = [];
    for(var i=0; i<route.length; i++) {
      lines.push([route[i].lng, route[i].lat]);
    }
    var matching = new Matching();
    var cPoints = matching.getCandidatePoints(msg.position.longitude, msg.position.latitude, null, [lines]);
    var point = cPoints[0].pop();
    console.log('point:' + point.longitude);

    id = msg.vehicle.id;
    pos = [msg.position.latitude, msg.position.longitude];
    mpos = [point.latitude, point.longitude];
    ...
    if (!(id in vehicles)) {
      ...
      micon = L.divIcon({
        className: "navigator-div-icon",
        iconAnchor: [14, 14],
        html: "<div id='vehicle-" + id + "' style='background: " + google_colors[routeType != null ? routeType : mode] + ";background-color:#ff0000;'><span>" + route + "</span><img src='static/images/" + google_icons[routeType != null ? routeType : mode] + "' height='20px' /></div>"
      });
      ...
      interpolation = function(index, id, old_pos) {
        ...
        var cPoints = matching.getCandidatePoints(lng, lat, null, [lines]);
        var point = cPoints[0].pop();
        vehicles[id].setLatLng([lat, lng]);
        mvehicles[id].setLatLng([point.latitude, point.longitude]);
        if (index < steps) {
